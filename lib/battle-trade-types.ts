...
export interface BroadcastSabotage {
  id: string
  type: 'BLACKOUT' | 'DARK_POOL' | 'REVERSAL'
  from: string
  to: string
  cost: number
...
export interface VolatilityEvent {
  type: 'CIRCUIT_BREAKER' | 'MOON_SHOT' | 'BLACKOUT' | 'WHALE_DUMP'
  asset?: 'BTC' | 'ETH' | 'SOL'
  impact?: number
  scheduledAt?: Date
}
...
export function generateMockData(): LobbyState {
  const traders: BroadcastTrader[] = [
    ...
    { id: '3', name: 'IRON HANDS', rank: 3, return: 24.5, balance: 12450, isEliminated: false, activity: 'idle', positions: [], sabotagesActive: [{ id: 's1', type: 'BLACKOUT', from: 'ANONYMOUS', to: 'IRON HANDS', cost: 200, remainingTime: 47, timestamp: new Date() }], credits: 180 },
  ...
  const sabotageEvents: SabotageEvent[] = [
    { id: 'se1', from: 'ANONYMOUS', to: 'WOLFPACK', type: 'BLACKOUT', cost: 200, timestamp: new Date(Date.now() - 30000) },
    { id: 'se2', from: 'VEGA', to: 'IRON HANDS', type: 'DARK POOL', cost: 150, timestamp: new Date(Date.now() - 120000) },
    { id: 'se3', from: 'WOLFPACK', to: 'DEGEN PRIME', type: 'REVERSAL', cost: 300, timestamp: new Date(Date.now() - 240000) },
    { id: 'se4', from: 'IRON HANDS', to: 'VEGA', type: 'BLACKOUT', cost: 200, timestamp: new Date(Date.now() - 360000) },
  ]
  ...
  return {
    ...
    predictionMarket,
    sabotageEvents,
    nextEvent: { type: 'CIRCUIT BREAKER', timeUntil: 134 },
  }
}
