...
// ---------------------------------------------------------------------------
const PRESETS = [
  { id: 'irl-event', name: 'IRL EVENT', desc: 'Conference / meetup — free entry, sponsor-funded credits', icon: '🎤', config: { starting_balance: 10000, round_duration_seconds: 300, entry_fee: 0, scoring_mode: 'best_round' as const, lobby_duration_minutes: 30, volatility_engine: 'manual' as const, credit_source: 'sponsor_funded' as const } },
  { id: 'quick-battle', name: 'QUICK BATTLE', desc: '15 min sprint — fast rounds, high leverage', icon: '⚡', config: { starting_balance: 10000, round_duration_seconds: 180, entry_fee: 0, scoring_mode: 'best_round' as const, lobby_duration_minutes: 15, volatility_engine: 'algorithmic' as const, credit_source: 'self_funded' as const } },
  { id: 'tournament', name: 'TOURNAMENT', desc: '1 hour elimination — entry fee, prize pool', icon: '🏆', config: { starting_balance: 10000, round_duration_seconds: 300, entry_fee: 500, scoring_mode: 'best_round' as const, lobby_duration_minutes: 60, volatility_engine: 'manual' as const, credit_source: 'self_funded' as const } },
  { id: 'custom', name: 'CUSTOM', desc: 'Configure everything yourself', icon: '⚙️', config: null },
];
...
type Step = 'preset' | 'config' | 'review' | 'done';
      ...
      <div style={{ minHeight: '100dvh', background: '#0A0A0A', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', borderBottom: '2px solid #1A1A1A', background: '#0D0D0D', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ ...B, fontSize: 18, color: '#888' }}>CREATE LOBBY</span>
          </div>
          {/* Step indicator */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            {(['preset', 'config', 'review', 'done'] as Step[]).map((s, i) => (
              <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <div style={{ width: 8, height: 8, background: step === s ? '#F5A0D0' : ['preset', 'config', 'review', 'done'].indexOf(step) > i ? '#00FF88' : '#222', boxShadow: step === s ? '0 0 8px rgba(245,160,208,0.5)' : 'none' }} />
                {i < 3 && <div style={{ width: 16, height: 1, background: '#222' }} />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', overflowY: 'auto', padding: '32px 24px' }}>
          <div style={{ width: '100%', maxWidth: 640 }} className="fade-in">
            ...
            {step === 'preset' && (
              <div>
                <h1 style={{ ...B, fontSize: 56, color: '#FFF', lineHeight: 1, textShadow: '0 0 40px rgba(245,160,208,0.2)' }}>CREATE A LOBBY</h1>
                <p style={{ ...S, fontSize: 14, color: '#888', marginTop: 8 }}>Choose a template to get started.</p>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, marginTop: 32 }}>
                  {PRESETS.map(p => (
                    <button
                      ...
                      style={{
                        padding: 24, background: '#0D0D0D', border: '2px solid #1A1A1A',
                        cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 8,
                        textAlign: 'left', transition: 'border-color 150ms',
                      }}
                      onMouseEnter={e => (e.currentTarget.style.borderColor = '#F5A0D0')}
                      onMouseLeave={e => (e.currentTarget.style.borderColor = '#1A1A1A')}
                    >
                      <span style={{ fontSize: 32 }}>{p.icon}</span>
                      <span style={{ ...B, fontSize: 24, color: '#FFF' }}>{p.name}</span>
                      <span style={{ ...S, fontSize: 12, color: '#888', lineHeight: 1.4 }}>{p.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {step === 'config' && (
              <div>
                <button onClick={() => setStep('preset')} style={{ ...B, fontSize: 14, color: '#555', background: 'none', border: 'none', cursor: 'pointer', marginBottom: 16 }}>← BACK</button>
                <h1 style={{ ...B, fontSize: 48, color: '#FFF', lineHeight: 1 }}>CONFIGURE</h1>
                <p style={{ ...S, fontSize: 13, color: '#888', marginTop: 4 }}>
                  {selectedPreset === 'custom' ? 'Set up your lobby from scratch.' : 'Tweak the defaults or go with them.'}
                </p>

                <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginTop: 24 }}>
                  {/* Lobby name */}
                  <div>
                    <label style={{ ...S, fontSize: 10, color: '#777', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>LOBBY NAME *</label>
                    <input
                      value={name}
                      onChange={e => setName(e.target.value.toUpperCase())}
                      placeholder="E.G. CONSENSUS MIAMI 2026"
                      maxLength={40}
                      style={{ width: '100%', height: 52, ...B, fontSize: 22, color: '#FFF', background: '#111', border: '2px solid #222', padding: '0 16px' }}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      {([
                        ...
                        <button key={f.key} onClick={() => setFormat(f.key)} style={{ flex: 1, padding: 16, background: format === f.key ? 'rgba(245,160,208,0.08)' : '#0D0D0D', border: `2px solid ${format === f.key ? '#F5A0D0' : '#1A1A1A'}`, cursor: 'pointer', textAlign: 'left' }}>
                          <span style={{ ...B, fontSize: 18, color: format === f.key ? '#F5A0D0' : '#FFF', display: 'block' }}>{f.label}</span>
                  ...
                  {/* Entry fee */}
                  <div>
                    <label style={{ ...S, fontSize: 10, color: '#777', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: 6 }}>ENTRY FEE (CREDITS)</label>
                    <div style={{ display: 'flex', gap: 4 }}>
                  ...
                  {/* Next button */}
                  <button
                    onClick={() => { if (name.trim()) setStep('review'); }}
                    disabled={!name.trim()}
                    style={{
                      width: '100%', height: 64, ...B, fontSize: 28,
                      color: name.trim() ? '#0A0A0A' : '#555',
                      background: name.trim() ? '#F5A0D0' : '#1A1A1A',
                      border: 'none', cursor: name.trim() ? 'pointer' : 'not-allowed',
                      textShadow: name.trim() ? '0 0 10px rgba(0,0,0,0.2)' : 'none',
                      marginTop: 8,
                    }}
                  >
                    REVIEW →
            ...
            {step === 'review' && (
              <div>
                <button onClick={() => setStep('config')} style={{ ...B, fontSize: 14, color: '#555', background: 'none', border: 'none', cursor: 'pointer', marginBottom: 16 }}>← BACK</button>
                <h1 style={{ ...B, fontSize: 48, color: '#FFF', lineHeight: 1 }}>REVIEW</h1>

                <div style={{ marginTop: 24, border: '2px solid #1A1A1A', background: '#0D0D0D' }}>
                  <div style={{ padding: '20px 24px', borderBottom: '1px solid #1A1A1A' }}>
                    <span style={{ ...B, fontSize: 36, color: '#F5A0D0' }}>{name}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0 }}>
                    {[
                      { label: 'FORMAT', value: format.toUpperCase() },
                      { label: 'ROUNDS', value: `${Math.floor(roundDuration / 60)} MIN EACH` },
                      { label: 'DURATION', value: `${lobbyDuration} MIN` },
                      { label: 'BALANCE', value: `$${startingBalance.toLocaleString()}` },
                      { label: 'LEVERAGE', value: leverages.map(l => `${l}x`).join(', ') },
                      { label: 'SCORING', value: scoringMode.replace('_', ' ').toUpperCase() },
                      { label: 'EVENTS', value: volatilityEngine.toUpperCase() },
                      { label: 'ENTRY FEE', value: entryFee === 0 ? 'FREE' : `${entryFee}CR` },
                      { label: 'VISIBILITY', value: isPublic ? 'PUBLIC' : 'PRIVATE' },
                    ].map(item => (
                      <div key={item.label} style={{ padding: '12px 16px', borderBottom: '1px solid #111', borderRight: '1px solid #111' }}>
                        <div style={{ ...S, fontSize: 9, color: '#888', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 2 }}>{item.label}</div>
                        <div style={{ ...M, fontSize: 14, color: '#FFF', fontWeight: 700 }}>{item.value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <button
                  ...
                  style={{
                    ...
                    color: '#0A0A0A', background: creating ? '#888' : '#00FF88',
                    border: 'none', cursor: creating ? 'not-allowed' : 'pointer',
                    textShadow: '0 0 10px rgba(0,255,136,0.3)',
                    boxShadow: creating ? 'none' : '0 0 24px rgba(0,255,136,0.2)',
                    marginTop: 24,
                  }}
                >
            ...
            {step === 'done' && createdLobby && (
              <div style={{ textAlign: 'center', paddingTop: 32 }}>
                ...
                <h1 style={{ ...B, fontSize: 64, color: '#00FF88', lineHeight: 1, textShadow: '0 0 40px rgba(0,255,136,0.4)' }}>LOBBY CREATED</h1>
                <p style={{ ...B, fontSize: 28, color: '#FFF', marginTop: 16 }}>{name}</p>

                <div style={{ marginTop: 32, padding: 24, border: '2px solid #F5A0D0', background: '#0D0D0D', display: 'inline-block' }}>
                  <div style={{ ...S, fontSize: 10, color: '#888', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.1em' }}>INVITE CODE</div>
                  <div style={{ ...M, fontSize: 40, color: '#F5A0D0', fontWeight: 700, letterSpacing: '0.15em', textShadow: '0 0 20px rgba(245,160,208,0.5)' }}>
                ...
                <div style={{ marginTop: 32 }}>
                  <div style={{ ...S, fontSize: 10, color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>SHARE LINKS</div>
